...
int getSpaceCount(char *string); /*ob`yavlenie funkcij*/
...
int main(void) {
    ...
    char * tmp;
    do {
        tmp = gets(string); /*schityvanie simvolov potokovogo vvoda-> sohranenie v stroku*/
        if (!tmp) break;
        trim(string);
...
double calculate(char *string) {
    ...
    while (1) {
        if(isEndOfLine(end)) {
            break; /*cikl dlya vihoda iz pererabotki stroki*/ 
        }
        string=end;
